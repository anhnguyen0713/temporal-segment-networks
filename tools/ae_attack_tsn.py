...
VIDEO_PATH = "/home/anhnguyen/action_recog/UCF-101/frames"
VIDEO_AE_PATH = "/home/anhnguyen/action_recog/UCF-101/tsn_fgsm_32_sample"
NET_PROTOTXT = "models/ucf101/tsn_bn_inception_rgb_train_val_ae_2.prototxt"
NET_WEIGHTS = "models/ucf101_split_1_tsn_rgb_reference_bn_inception.caffemodel"
OUTPUT = "results/rgb_center_fgsm_8_rate_1.txt"
...
def predict_batch(data_in, label, calculate_grad=False):
	global net

	# Generate label blob
	labels_in = np.zeros((BATCH_SIZE,1,1,1), dtype=np.uint8)
	labels_in[:] = label
...
for i_line, line in enumerate(f_lines):
	...
	for idx,frame_path in enumerate(selected_frames):
		...
		if(len(batch_data) == BATCH_SIZE):
			...
			# Predict clean batch and calculate gradient w.r.t data layer
			score_batch, grad = predict_batch(batch_data_in, label, calculate_grad=True)
			score_batch = np.squeeze(score_batch)

			if ATTACK:
				# FGSM
				signed_grad = np.sign(grad)
				pertub = signed_grad * EPS
				...
				# Predict ae batch
				score_batch = predict_batch(ae_batch, label, calculate_grad=False)
				score_batch = np.squeeze(score_batch)

				# save_ae_frames(process_fr_names, ae_batch, batch_data)	
